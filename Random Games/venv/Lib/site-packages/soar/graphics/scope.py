...
class Oscilloscope():
    ...
    def makeCanvasObjects(self):
        ...
        # number of text objects doesn't change with rescale
            ...
            self.xmax_text = self.canvas.create_text(0, 0, text="")
            self.yminText = self.canvas.create_text(0, 0, text="")
            self.ymax_text = self.canvas.create_text(0, 0, text="")
    ...
    def add_probeFunction(self, names, func, functext=None, initVal=0):
        ...
        if type(names) == list or type(names) == tuple:
            ...
            for i in range(numprobes):
                ...
                text = text+"["+str(i)+"]"
                self.add_probe(names[i], self.makeFunc(func, i), text, initVal)
        else:
            self.add_probe(names, self.makeFunc(func), 
                          functext if functext else str(func), initVal)
        self.setStaticObjects()

    def add_probe(self, name, functext, initVal=0):
        func = lambda: eval(functext)
        self.add_probeFunction(name, func, functext, initVal)

    def setProbeText(self, probeidx, name, functext):
        func = lambda: eval(functext)
        self.setProbe(probeidx, name, func, functext)

    def add_probe(self, name, func, functext, initVal=0):
        ...
        if self.opts['Y Scale'] != 'auto':
            ...
            def f():
                self.setCanvasText(self.yminText, x_gapl/2, c_height-y_gap, 
                                   text=round(smin,3))
                self.setCanvasText(self.ymax_text, x_gapl/2, y_gap, 
                                   text=round(smax,3))
            text_funcs.append(f)
        def f():
            # x scale labels
            self.setCanvasText(self.xminText, x_gapl, c_height-y_gap/2, 
                               text=self.xrange)
            self.setCanvasText(self.xmax_text, c_width-x_gapr, c_height-y_gap/2, 
                               text="0")
            # axes labels
